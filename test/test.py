...
# Set Qt to offscreen mode before importing cv2 (prevents display errors in headless mode)
...
# ---- Serial ----
# Using udev symlinks for persistent device names (see 99-air-hockey.rules)
# Using udev symlinks for persistent device names (see 99-air-hockey.rules)
SERIAL_PORT = "/dev/motor_controller"      # Motor controller (RP2350)
FAN_SERIAL_PORT = "/dev/fan_controller"    # Fan/Solenoid controller
BAUDRATE = 115200
...
# ---- Goal detection ----
# Goals are on the left/right sides of the camera view (X axis in camera space)
# If puck was near an edge and moving toward it, then disappears = goal
GOAL_EDGE_THRESHOLD = 100   # pixels from edge to consider "near goal"
PUCK_LOST_FRAMES = 5        # frames puck must be missing to confirm goal
GOAL_COOLDOWN = 3.0         # seconds to wait after goal before detecting another
...
def send_fan_command(command: str, wait_for_boot: bool = False):
    """
    Connect to Fan controller and send a command.
    Used for READY (after homing) and RESET (after goal).
    
    wait_for_boot: If True, wait longer for MCU to boot after serial open
                   (opening serial port can reset the MCU via DTR line)
    """
    try:
        # Open port without DTR to avoid resetting the MCU
        fan_ser = serial.Serial()
        fan_ser.port = FAN_SERIAL_PORT
        fan_ser.baudrate = BAUDRATE
        fan_ser.timeout = 1.0
        fan_ser.dtr = False  # Don't toggle DTR (prevents MCU reset)
        fan_ser.open()
        
        if wait_for_boot:
            time.sleep(2.5)  # Wait for MCU to fully boot if it did reset
        else:
            time.sleep(0.1)  # Brief settle time
            
        cmd = command.strip() + "\n"
        fan_ser.write(cmd.encode("utf-8"))
        fan_ser.flush()
        print(f"[Fan] Sent {command}")
        fan_ser.close()
        return True
    except Exception as e:
        print(f"[Fan] Failed to send {command}: {e}")
        return False


def send_fan_ready():
    """
    Send READY command to Fan controller.
    This enables coin insertion after motor homing is complete.
    """
    print(f"[Fan] Opening {FAN_SERIAL_PORT}...")
    return send_fan_command("READY", wait_for_boot=True)


def send_fan_reset():
    """
    Send RESET command to Fan controller after a goal is scored.
    This turns off the fan and disables coin insertion until next READY.
    """
    return send_fan_command("RESET")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="PID Puck Tracking")
    parser.add_argument("--skip-home", action="store_true",
                        help="Skip homing sequence (use when daemon already homed)")
    parser.add_argument("--headless", action="store_true",
                        help="Run without display (for daemon/service mode)")
    ...
    # Blocking HOME sequence (skip if already homed by daemon)
    if args.skip_home:
        print("[Main] Skipping homing (--skip-home flag set)")
    else:
    ...
    # Homing complete - enable coin insertion on Fan controller
    send_fan_ready()
    ...
    try:
        while True:
            ...
            # Goal detection: puck was near edge and now disappeared
            if puck_pos is None and prev_puck_pos is not None:
                frames_puck_lost += 1
                
                # Check if puck was near a goal edge (left or right side of camera)
                if frames_puck_lost >= PUCK_LOST_FRAMES:
                    px = prev_puck_pos[0]
                    near_left_goal = px < (x_min + GOAL_EDGE_THRESHOLD)
                    near_right_goal = px > (x_max - GOAL_EDGE_THRESHOLD)
                    
                    # Only trigger if enough time has passed since last goal
                    if (near_left_goal or near_right_goal) and (now - last_goal_time) > GOAL_COOLDOWN:
                        side = "LEFT" if near_left_goal else "RIGHT"
                        print(f"[Goal] GOAL DETECTED on {side} side! Sending RESET...")
                        motor.stop()
                        send_fan_reset()
                        last_goal_time = now
                        prev_puck_pos = None  # Clear so we don't re-trigger
                        frames_puck_lost = 0
            elif puck_pos is not None:
                # Puck is visible - reset lost counter and update previous position
                frames_puck_lost = 0
